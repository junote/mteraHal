...
class PMBus(I2cBus):
    ...
    def _decodePMBus(self, message):
        ...
        # calculate real values (everything but VOUT works)
        message = messageY*(2.0**(self.twos_comp(messageN, 5)))
        return message

    def _encodePMBus(self, message):
        YMAX = 1023.0
        #print(message)
        Nval = int(log2(message/YMAX, 2))
        #print("NVal: " + str(Nval))
    ...
    def _writeWordPMBus(self, cmd, word):
        writeBuf = [self.addr, cmd, (word & 0xff), (word >> 8)]
        self.i2cWrite(writeBuf)


    def _readWordPMBus(self, cmd):
        # write_buf = i2c_msg.write(self.addr, [cmd])
        # read_buf = i2c_msg.read(self.addr, 2)
        # with SMBusWrapper(self.busID) as bus:
        #     bus.i2c_rdwr(write_buf, read_buf)
        # return ord(read_buf.buf[0])+(ord(read_buf.buf[1]) << 8)
        writeBuf1 = [self.addr, cmd]
        writeBuf2 = [self.addr]
        readBuf = [0, 0]
        retBuf = self.i2cRepeatStartRead(writeBuf1,writeBuf2,readBuf)
        return retBuf[0] + (retBuf[1] << 8)

    def _writeBytePMBus(self, cmd, byte):
        # write_buf = i2c_msg.write(self.addr, [cmd, byte])
        # with SMBusWrapper(self.busID) as bus:
        #     bus.i2c_rdwr(write_buf)
        writeBuf = [self.addr, cmd, byte]
        self.i2cWrite(writeBuf)        

    def _readBytePMBus(self, cmd):
        # write_buf = i2c_msg.write(self.addr, [cmd])
        # read_buf = i2c_msg.read(self.addr, 1)
        # with SMBusWrapper(self.busID) as bus:
        #     bus.i2c_rdwr(write_buf, read_buf)
    ...
    def setVinUVLimit(self, uvWarnLimit, uvFaultLimit):
    ...
    def setVinOVLimit(self, ovWarnLimit, ovFaultLimit):
    ...
    def setVoutOVLimit(self, ovWarnLimit, ovFaultLimit):
    ...
    def setIoutOCLimit(self, ocWarnLimit, ocFaultLimit):
    ...
    def setOTLimit(self, otWarnLimit, otFaultLimit):
    ...
    def setFaultResponse(self, register, byte):
        ...
        print "Fault Response: " , bin(self.getFaultResponse(register))
    ...
    def getCurrentIn(self):
        self.currentIn = self._decodePMBus(self._readWordPMBus(0x89))
        return self.currentIn

    def getCurrentOut(self):
        self.currentOut = self._decodePMBus(self._readWordPMBus(0x8C))
        return self.currentOut

    def getPowerIn(self):
        self.powerIn = self._decodePMBus(self._readWordPMBus(0x97))
        return self.powerIn
